...
6. **`/track`** — whenever `/next` routes to it. Closes the issue if every box is ticked, refreshes the work mirror, and commits that refresh **straight to the default branch** where the branch allows it — no branch, no pull request (`AGENTS.md`, *Git and delivery*), because a pull request per refresh is what made this loop back on itself. Where protection forbids that push, the same rule has the refresh ride along with the next substantive pull request instead, never its own.
