...
No `.claude/commands/*-local.md` companions exist yet — this is a first install of the kit into
this repository (`.claude/kit.json` records the installed commit). The Commands/Architecture/
...
This repository has no `design/00-brief.md`-driven feature yet beyond the seed docs installed with
the kit — `design/` currently holds only the kit's template skeleton (`00-brief.md`, `10-design.md`,
`20-contract.md`, `30-slices.md`, `90-decisions.md`), none of it filled in. Once populated, the
design docs outrank the code, in precedence order:
...
- **No work otherwise lands directly on the default branch, ever — not even a doc or contract edit made outside a named slash command.** Before the first edit of any change, create a fresh branch off the default branch if one isn't already checked out. **One narrow exception exists, and it is not about weight — it is about reviewability.** A *derived design-state record* is generated, deterministic, and already checked by `tools/Test-DesignState.ps1`; a pull request over one is review theatre, and that theatre is what closes the loop below. Such a change is committed and pushed **straight to the default branch**, opening no pull request, when **all** of these hold: every staged path is under `design/state/work/` or is `design/state-index.md`; every one of them was written by `tools/Update-WorkMirror.ps1` or `tools/Update-DesignProjection.ps1` in this same run; `git status` shows nothing else modified; and `tools/Test-DesignState.ps1` was run afterwards and reported no blocking finding. **Any other path on the diff voids the exception for the whole commit** — branch and open a pull request as normal, carrying the records along with the rest. The reason this exception exists is the loop it breaks: the work mirror mirrors GitHub, which is externally mutable and therefore has no fixed point, so a pull request per refresh means a merge per refresh, and a merge is what puts `/clean` back on the table, which hands back to `/track`, which refreshes the mirror again. **The exception does not apply where the default branch is protected in a way a direct push cannot satisfy**, which is this repository's case: the `Main` ruleset requires the `build` and `machine-state` contexts, so a push straight to `main` is rejected with `2 of 2 required status checks are expected` however cleanly the four conditions above are met — *CI and releasing* states the same fact from the other side. **A refresh there rides along with the next substantive pull request; it never gets one of its own.** That is the same loop-breaking intent reached by the only route left open, because the failure mode is not the pull request but the pull request *per refresh* — a mirror record carried on a branch that was going to merge anyway adds no merge that was not already coming. The refresh is therefore left uncommitted until there is something to carry it, and a run that finds nothing to ride along with says so rather than opening a bookkeeping pull request. **Do not weaken branch protection to make the direct-push path work**; the protection is the more valuable of the two rules.
...
- Commit messages state what changed and which slice it belongs to. No AI attribution in commits or PR bodies — see *CI and releasing* below, which already states this repository's own version of the rule; this is the one instance where the target's existing wording wins over the kit's and the duplicate is dropped rather than kept in both places.
