...
- **GitHub Packages** — automatic, on two triggers. A **push to `main`** (every merged PR) publishes a distinct **prerelease** `0.2.0-<n>`; pushing a **`v*` tag** (`git push origin v0.2.0`) publishes the **stable** `0.2.0`. The version comes from [GitVersion](https://gitversion.net/) via [GitVersion.yml](https://github.com/The-Running-Dev/SubZeroDev.WinGet/blob/main/GitVersion.yml), which derives it from git history rather than the `.csproj`. Auth uses the built-in `GITHUB_TOKEN`, so no secret setup is needed.
