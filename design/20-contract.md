...
| C3 | Evidence is identified by gate, environment, and commit. It records outcome, run identity, asserted facts, and explicit non-assertions. A passing subset licenses only the claims asserted by that subset. | Live-verification workflow | Enforced in the `build`, `machine-state`, and `catalog` jobs' evidence steps in `.github/workflows/build.yml`, each recording commit, run identity, outcome, and explicit `Non-assertion` lines, and each uploading its record as a retained artifact. Which subject a given assertion licenses is authored in those steps and review-enforced. |
...
| C6 | A gate exists only when a workflow invokes it, can fail on a violated assertion, and records a binary outcome for the exact commit tested. A target that merely reports a value is not a gate. | Build and workflows | Enforced in workflow composition: every target named under *Build and workflow commands* except the local aggregate `IntegrationTest` is invoked by a job in `.github/workflows/build.yml`. `IntegrationTest`, `CoverageGateTest`, `LiveTestCountGateTest`, and `PublicationConfirmationTest` are deliberately uninvoked and are therefore not gates. |
...
| C8 | `MachineStateTest` selects exactly seven machine-state tests, `CatalogIntegrationTest` selects exactly six catalog-dependent tests, and both fail before execution if their stable risk metadata selects another count, and after execution unless exactly that many tests passed — a selected test skipped, ignored, or never executed fails the target rather than leaving it green. The machine-state target and `PackedConsumerSmokeTest` share one required pull-request status; the catalog target has a separately failing, non-required status. `IntegrationTest` is only their local aggregate. | Live-verification workflow | Count assertions enforced by `Build.cs`'s `AssertLiveTestCount` before execution and by `LiveTestCountGate` (`build/LiveTestCountGate.cs`) over the trx `passed` counter after it, whose boundary cases the local `LiveTestCountGateTest` target covers; the `machine-state`/`catalog` job split is enforced in workflow composition. The machine-state status is marked required in branch protection, and the catalog status is not: the `Main` repository ruleset requires exactly the `build` and `machine-state` contexts. |
| C9 | `PackedConsumerSmokeTest` is separate from `PackageTest`. Both reuse one consumer-construction implementation, but only the smoke target executes the packed consumer and activates COM. | Build | Enforced in `build/Build.cs`: `PackageTest` calls `PackageVerification.Run` and `PackedConsumerSmokeTest` calls `PackageVerification.RunPackedConsumerSmoke`, and the two targets are invoked by different jobs. |
...
| C14 | Every CsWinRT-projected collection is traversed by index. Exposing translation functions to tests must not replace indexed loops with `foreach` or LINQ. | Client translation boundary | Enforced by production code and by the forbidden-traversal-token check in `WinGetProjectionMapperArchitectureTests.cs`, which fails on `foreach` appearing in the mapper source. |
| C15 | A failed operation whose extended HRESULT is `WinGetErrorCodes.InstallCancelledByUser` produces `PackageOperationStatus.Cancelled`, regardless of the projection status enum's lack of a cancelled member. Caller-token cancellation remains an `OperationCanceledException`; the two outcomes are not normalized together. | Client translation boundary | Enforced in `WinGetProjectionMapper.ToOperationResult` and by `WinGetProjectionMapperOperationResultTests.cs`, which pins both the cancelled mapping and the retained HRESULT. |
| C16 | `GetWinGetVersion` returns `null` only when the runtime does not expose the version member through the supported activation path. Likewise, a source's priority reads as `0` only when the runtime does not expose the contract-29 priority member. In both, cancellation and every unrelated failure remain distinguishable and propagate according to *Error semantics*. | Client layer and client translation boundary | Version: enforced by the `ENoInterface`-only guard in `WinGetClient.GetWinGetVersionCore`, by `WinGetClientVersionTests.cs`, and live by `GetWinGetVersion_ReturnsAVersion` in the machine-state class. Priority: enforced by the `ENoInterface`-only guard in `WinGetProjectionMapper.ReadPriority` and by `WinGetProjectionMapperPriorityTests.cs`; `GetPriority`'s projection read itself stays licensed live under C26. |
| C17 | `WinGetActivationModeSelector` tries projection, local server, then local server with lower-trust registration. The first successful mode is reused for later activations in that factory; a cached mode's later failure propagates without reselection; total initial failure becomes `WinGetUnavailableException` with every attempt failure retained; a failure is not cached as a successful mode. | COM activation layer | Enforced by production code and by the five unit tests in `WinGetActivationModeSelectorTests.cs`, which cover mode ordering with first success, cached-mode reuse, cached-failure propagation without reselection, total-failure aggregation with a later retry, and serialized selection across concurrent first callers. |
...
| C19 | A live run with partial success records evidence for each passing assertion and failure for each failing assertion. It does not become an all-suite pass, and vacuous assertions over empty inputs license no claim. | Live-verification workflow | Enforced by the per-test trx and detailed console loggers on both live targets in `Build.cs`, which retain a per-assertion record rather than one job outcome, by `LiveTestCountGate`, which fails either live target whose trx records fewer passes than its C8 count so a run with skipped or unexecuted tests cannot report as an all-suite pass, and by `AssertCopiedOrNonAssertion` in `WinGetClientIntegrationTests.cs`, whose `Non-assertion:` lines the `catalog` job's evidence step reads back into the record. |
...
| C21 | A release is identified by its pushed tag and exact commit. `v0.2.0` is the next stable release; `v0.1.0` is not moved. A publishing run is successful only after the intended version is positively confirmed on each intended feed. | Release workflow | Tag immutability is repository policy — the `Main` ruleset's `deletion` and `non_fast_forward` rules cover the branch, not tags. Positive feed confirmation is enforced by `PublicationConfirmation.Confirm`/`Assert` on both `PublishNuGet` and `PublishGitHubPackages` (`build/Build.cs`), with the comparison itself covered by `PublicationConfirmationTest`. |
| C22 | No work governed by this contract adds public library surface, changes the service/client/COM dependency direction, or broadens the documented CLI-shim exception beyond pins and export/import. | Product | Enforced by `ProductInvariantTests.cs`: the public surface is compared against a checked-in baseline, the client, activation, and mapper sources are checked for back-edges against the layer above them, `IWinGetCliClient` is pinned to exactly the pin/export/import members, and process creation is confined to the CLI shim. |
| C23 | The machine-state live status is not made required until the narrowed version-member classifier has run on the GitHub-hosted Windows x64 environment constituted by C24 and `PackedConsumerSmokeTest` has observed a non-null version. A `null` result or unrelated failure stops that rollout; it does not license a fallback observable, weaker assertion, or required status. | Live-verification workflow | Precondition satisfied: run 33671049835 (commit `44afa873`, 2026-09-02) observed a non-null WinGet version (1.29.280) from the packed consumer on GitHub-hosted Windows x64. The rollout then completed: the `Main` repository ruleset was updated on 2026-09-03 to require the `machine-state` context. That setting lives in repository configuration rather than in this tree, so it is verified by reading the ruleset, not by a check in this repository. |
| C24 | The hosted live-verification jobs constitute their own WinGet runtime: they install one explicitly pinned build — the version the library's `Microsoft.WindowsPackageManager.ComInterop` reference pins — and record the App Installer version observed both before and after that install. A floating latest is not a pinned build, because C4's observed environment fact must be reproducible. Constituting the runtime is not a fallback observable: the assertion under C23 remains a non-null `GetWinGetVersion` from the packed consumer. | Live-verification workflow | Enforced in the `machine-state` and `catalog` jobs in `.github/workflows/build.yml`: each installs the `WINGET_VERSION` build pinned in the workflow `env` to the library's `Microsoft.WindowsPackageManager.ComInterop` reference, fails if App Installer is absent afterwards, and records the version observed before and after into its evidence record. |
...
`ReadPriority(Func<int>)`, the `E_NOINTERFACE` classifier `GetPriority` delegates to, is neither an
eleventh live-licensed member nor the stub-backed read accessor C26 forbids. Its input is an ordinary
delegate, and its unit tests assert which exceptions become `0` and which propagate — a claim about
the classifier, never about what the projection reads. That is the `ReadWinGetVersion` shape C16
already relies on. `GetPriority` itself, the one-line `() => info.Priority` wrapper, stays
live-licensed and without a unit test.
...
| Priority member unavailable | Reading a source's priority throws `InvalidCastException` with `HResult == 0x80004002` (`E_NOINTERFACE`) because the runtime predates contract 29. Returned as priority `0`, not thrown. | No automatic retry in the same environment. | Treat the priority as the default; do not infer that the source is unusable. |
| Unexpected priority access failure | Reading a source's priority fails for any reason outside the unavailable-member classifier. The original typed exception propagates from the source call that projected it. | Determined by the concrete exception; the client adds no retry. | Diagnose the concrete failure. It must remain distinguishable from a genuine `0`. |
...
No other exception or HRESULT means that the version or priority member is unavailable. A bare
`catch`, message match, normalization of every `InvalidCastException`, or normalization of any
`COMException` to `null` (or, for priority, to `0`) violates C16.
