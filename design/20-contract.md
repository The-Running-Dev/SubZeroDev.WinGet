...
| C16 | `GetWinGetVersion` returns `null` only when the runtime does not expose the version member through the supported activation path. Cancellation and every unrelated failure remain distinguishable and propagate according to *Error semantics*. | Client layer | Enforced by the `ENoInterface`-only guard in `WinGetClient.GetWinGetVersionCore`, by `WinGetClientVersionTests.cs`, and live by `GetWinGetVersion_ReturnsAVersion` in the machine-state class. |
