...
| 6 | **First GitHub Packages publish is done, and a stable version has been released.** `0.0.1-8` was pushed to the feed on the 2026-07-22 merge, which exposed that GitVersion derives the version from git history and ignores the `.csproj` `<Version>`. The same day, the annotated `v0.1.0` tag was pushed at commit `c2cc157`; that tag push ran the `release` job to success, publishing the stable `0.1.0` package (design/90-decisions.md, 2026-08-20). Untagged commits on main have since published further prereleases (e.g. `0.1.1-<n>`, run 32406423223). [`GitVersion.yml`](GitVersion.yml) now pins `next-version: 0.2.0` — `v0.1.0` is not moved, and `v0.2.0` is the next stable release. Both `PublishGitHubPackages` and `PublishNuGet` now also positively confirm the intended version back from the feed they pushed to before the target succeeds, rather than trusting a push exit code or a skipped duplicate (S11, `design/20-contract.md` C21). |
| 7 | First **NuGet.org** publish (separate from GitHub Packages): set the `NUGET_API_KEY` secret and run the workflow with `push_to_nuget`. Publishes the `.csproj`-pinned version. This path has not run yet. |
| 8 | The Nuke build (§9) is validated locally **and** in GitHub Actions. `PublishGitHubPackages` has since been exercised for real (item 6); `PublishNuGet` remains unexercised (item 7). The build system is expected to be replaced shortly, at which point §9 and the `build/` project go with it. |
