...
**A bug fixed in passing:** the original NuGet.org publish step passed `--skip-duplicates` (plural) to `dotnet nuget push`; the real flag is singular (`--skip-duplicate`). This went uncaught for a long time because that path did not execute for real until S12's `v0.2.0` release (§11, item 7). The Nuke port's correct singular flag via `.EnableSkipDuplicate()` is what ran and published successfully.

**Validation status:** the Nuke build has been compiled and executed on a real Windows host and in GitHub Actions (2026-07-21). `nuke Test Coverage Pack --configuration Release` completes green through the global tool — the exact invocation path CI uses — producing `artifacts/SubZeroDev.WinGet.0.1.0.nupkg`, `coverage/Cobertura.xml`, and `coverage/SummaryGithub.md`, with `[GitVersion]` injecting cleanly. `PublishGitHubPackages` has since run for real: the 2026-07-22 merge published a prerelease, and the `v0.1.0` tag pushed the same day published the stable version (§11, item 6). `PublishNuGet` has since run for real too: S12's manually dispatched `push_to_nuget` run against the `v0.2.0` tag published to NuGet.org and positively confirmed the version on the feed (§11, item 7).
...
| 6 | **`v0.2.0` is the proven, tagged release; both feeds have positively confirmed it (S12).** The annotated `v0.2.0` tag was pushed at commit `3b99a93` on 2026-09-05; `v0.1.0` (`c2cc157`, item unchanged below) is not moved. The tag push's `Build` run ([33960128193](https://github.com/The-Running-Dev/SubZeroDev.WinGet/actions/runs/33960128193)) published GitHub Packages and printed `Publication confirmed: GitHub Packages 0.2.0 (tag/ref v0.2.0, commit 3b99a93…)`. A manually dispatched `workflow_dispatch` run with `push_to_nuget: true` ([33960655434](https://github.com/The-Running-Dev/SubZeroDev.WinGet/actions/runs/33960655434)) published to NuGet.org and printed `Publication confirmed: NuGet.org 0.2.0 (tag/ref v0.2.0, commit 3b99a93…)`. `3b99a93`'s own machine-state/catalog evidence is borrowed by tree identity from PR #90's pre-squash head (`2d3e048`) rather than run against that exact SHA — `.github/workflows/build.yml`'s `machine-state`/`catalog` jobs run only on `pull_request`, and this repository squash-merges, so no commit on `main` ever carries its own such run; see design/90-decisions.md, 2026-09-05, for the full reasoning and its known non-assertion. First GitHub Packages publish was `0.0.1-8` on the 2026-07-22 merge (exposing that GitVersion derives the version from git history, ignoring the `.csproj` `<Version>`); the stable `0.1.0` published the same day at `c2cc157` (design/90-decisions.md, 2026-08-20). `GitVersion.yml` still pins `next-version: 0.2.0` as of this tag — bumping it to the next version being worked toward is out of S12's scope (not in its `Touches` list) and is left for whatever slice starts the next release cycle. Both `PublishGitHubPackages` and `PublishNuGet` positively confirm the intended version back from the feed they pushed to before the target succeeds, rather than trusting a push exit code or a skipped duplicate (S11, `design/20-contract.md` C21). |
| 7 | **First NuGet.org publish is done** (item 6) — `push_to_nuget` was run against the `v0.2.0` ref and positively confirmed `0.2.0` on the feed. |
| 8 | The Nuke build (§9) is validated locally **and** in GitHub Actions. Both `PublishGitHubPackages` and `PublishNuGet` have now been exercised for real and positively confirmed (item 6). The build system is expected to be replaced shortly, at which point §9 and the `build/` project go with it. |
