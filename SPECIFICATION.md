...
| [website/](website) | The Docusaurus site that builds `docs/` (via `path: '../docs'`) and deploys to GitHub Pages on push to `main` (`.github/workflows/docs.yml`). Kept separate from the main `.sln`/CI so a docs change never touches library CI and vice versa. |
