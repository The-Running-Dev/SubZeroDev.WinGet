...
- **GitHub Packages** — **automatic on every push to `main`** (i.e. every merged PR). The version comes from [GitVersion](https://gitversion.net/) via [GitVersion.yml](GitVersion.yml), which derives it from git history rather than the `.csproj`: an untagged commit publishes a distinct prerelease (`0.1.0-<n>`), and tagging a commit (`v0.1.0`) publishes the stable version. Auth uses the built-in `GITHUB_TOKEN`, so no secret setup is needed.
